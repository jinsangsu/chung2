...
def load_qa_sheet():
    try:
        sheet_url = "https://docs.google.com/spreadsheets/d/1rJdNc_cYw3iOkOWCItjgRLw-EqjqImkZ/export?format=csv"
...
if prompt := st.chat_input("궁금한 내용을 입력해 주세요"):
    ...
    for _, row in qa_df.iterrows():
        question = str(row["질문 내용"]).strip().lower()
        if prompt_lower in question or question in prompt_lower:
